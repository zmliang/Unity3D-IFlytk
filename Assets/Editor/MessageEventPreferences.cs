    ...
    public static class MessageEventPreferences
    {
        ...
        private static void SaveMessageEventSettings()
        {
            ...
            foreach (KeyValuePair<string, List<MessageEventEntry>> pair in groupedEntries)
            {
                ...
                sw.WriteLine("namespace JinkeGroup.MessageEvents {");
                ...
                {
                    MessageEventEntry e = pair.Value[a];
                    string s = string.Format("            JinkeGroup.Logic.MessageEventManager.MessageEventsInfo.Add({0}, \"{1}.{2}\");", e.Hash, masterEntry.Namespace, e.Name);
                    sw.WriteLine(s);
                }
                sw.WriteLine("        }");
                sw.WriteLine("#endif");
