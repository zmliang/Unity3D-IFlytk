...
namespace JinkeGroup.Util
{
    public static class UserPrefs
    {
        ...
        private const string EmptyCollectionValue = "Empty Collecition Value";
        ...
        {
            Logger.WarnT(Tag, "Clearning all user preferences");
        ...
        {
            Logger.VerboseT(Tag, "Saving all user preferences");
        ...
        {
            Logger.VerboseT(Tag, "Saving delayed for {0} second(s), savePending={1}", delaySecs, SavePending);
            ...
            {
                Logger.WarnT(Tag, "Delayed save cannot be accomplished because MainExecutor is null");
                Save();
            }
        }
        ...
        public static long GetLong(string key, long defaultValue)
        {
            if (HasKey(key))
            {
                ...
                {
                    Logger.WarnT(Tag, "Invalid Int64 format for value '{0}' for key '{1}'", value, key);
                    return defaultValue;

                }
                catch (OverflowException)
                {
                    Logger.WarnT(Tag, "Overflow of Int64 for value '{0}' for key '{1}'", value, key);
                    return defaultValue;
                }
            }
            return defaultValue;
        ...
        public static double GetDouble(string key, double defaultValue)
        {
            if (HasKey(key))
            {
                ...
                {
                    Logger.WarnT(Tag, "Invalid Double format for value '{0}' for key '{1}'", value, key);
                    return defaultValue;

                }
                catch (OverflowException)
                {
                    Logger.WarnT(Tag, "Overflow of Double for value '{0}' for key '{1}'", value, key);
                    return defaultValue;
                }
            }
            return defaultValue;
        ...
        public static long? SetLongAndReturnPrevious(string key, long value)
        {
            ...
            if (HasKey(key))
            {
                ...
                {
                    Logger.WarnT(Tag, "Invalid Int64 format for value '{0}' for key '{1}'", prevValueS, key);

                }
                catch (OverflowException)
                {
                    Logger.WarnT(Tag, "Overflow of Int64 for value '{0}' for key '{1}'", prevValueS, key);
                }
            }
            SetLong(key, value);
            return prevValue;
        ...
        public static double? SetDoubleAndReturnPrevious(string key, double value)
        {
            ...
            if (HasKey(key))
            {
                ...
                {
                    Logger.WarnT(Tag, "Invalid Double format for value '{0}' for key '{1}'", prevValueS, key);

                }
                catch (OverflowException)
                {
                    Logger.WarnT(Tag, "Overflow of Double for value '{0}' for key '{1}'", prevValueS, key);
                }
            }
            SetDouble(key, value);
            return prevValue;
