...
namespace JinkeGroup.Util
{
    ...
    public partial class AssetReference
    {
        ...
        public UnityEngine.Object Load(Type type)
        {
            ...
            if (InSceneLoading)
            {
                JinkeGroup.Util.Logger.Error("Trying to load resource {0} sync while loading scene!", PrefabPath);
            }
#endif

            var resource = ResourceManager.Load(PrefabPath, type);
        ...
        public ResourceRequest LoadAsync(Type type)
        {
            ...
            if (InSceneLoading)
            {
                JinkeGroup.Util.Logger.Error("Trying to load resource {0} async while loading scene!", PrefabPath);
            }
#endif
            return ResourceManager.LoadAsync(PrefabPath, type);
        }
