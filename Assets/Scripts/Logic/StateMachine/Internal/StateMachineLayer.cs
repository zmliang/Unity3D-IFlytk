        ...
        private Transition CheckStateTransitions(StateMachine stateMachine, State state)
        {
            for (int i = 0; i < state.TransitionIndices.Length; i++)
            {
                ...
                for (int j = 0; j < transition.Conditions.Length; j++)
                {
                    ...
                    {
                        JinkeGroup.Util.Logger.ErrorT(stateMachine.Tag, "Condition {0} in state {1} transition {2} invalid!", condition.ParameterIndex, state.Name, transition.DestinationState.Name);
                        continue;
                    }
#endif
                    if (!condition.IsTrue())
                ...
                while (true)
                {
                    if (canIterate && (CurrentState == null || CurrentState.Switch || CurrentTransition == null || !CurrentTransition.Atomic))
                    {
                        ...
                        if (nextTransition != null)
                        {
                            ...
                            {
                                JinkeGroup.Util.Logger.DebugT(stateMachine.Tag, "{0} Layer:{1} From:{2} To:{3}", stateMachine.name, Name, PreviousState != null ? PreviousState.Name : "None", CurrentState.Name);
                            }
                        }
                        else
                        {
