...
using JinkeGroup.Logic.MessageEventInternal;
...
namespace JinkeGroup.Logic
{
    public class MessageEventManager : Manager<MessageEventManager>
    {
        ...
        public void Post(int eventHash, int intData0 = 0, int intData1 = 0, float floatData0 = 0.0f,
            float floatData1 = 0.0f, UnityEngine.Object objectData0 = null, object objectData1 = null,
            object userData = null, int lockMask = 0, object sender = null)
        {
#if !STRIP_TEST
            if (BlockPostEvents)
            {
                JinkeGroup.Util.Logger.DebugT(Tag, "Post: {0} blocked by running test!", eventHash);
                return;
            }
#endif
            ...
            {
                JinkeGroup.Util.Logger.DebugT(Tag, "Post: {0} locked!", eventHash);
                return;
            }
            // Get new event
...
#if DEVEL_BUILD || PROD_BUILD || UNITY_EDITOR
            string desc = MessageEventManager.Instance.GetMessageEventDesc(messageEvent);
            JinkeGroup.Util.Logger.DebugT(Tag, "Post: {0}", desc);
#endif
        }
        ...
        public void PostNow(int eventHash, int intData0 = 0, int intData1 = 0, float floatData0 = 0.0f, float floatData1 = 0.0f, UnityEngine.Object objectData0 = null, object objectData1 = null, object userData = null, int lockMask = 0, object sender = null)
        {
            ...
            if (!SetLockMask(lockMask))
            {
                JinkeGroup.Util.Logger.DebugT(Tag, "PostNow: {0} locked!", eventHash);
                return;
            }
            // Get new event
            MessageEvent messageEvent = GetEvent();
...
#if DEVEL_BUILD || PROD_BUILD || UNITY_EDITOR
            string desc = MessageEventManager.Instance.GetMessageEventDesc(messageEvent);
            JinkeGroup.Util.Logger.DebugT(Tag, "PostNow: {0}", desc);
#endif
        ...
        public void LockGlobal(int bitIndex)
        {
            ...
            {
                JinkeGroup.Util.Logger.DebugT(Tag, "LockGlobal bitIndex {0}, refCount {1}", bitIndex, GlobalLockRefCount[bitIndex]);
            }
        }
        ...
        public void UnlockGlobal(int bitIndex)
        {
            ...
            {
                JinkeGroup.Util.Logger.DebugT(Tag, "UnlockGlobal bitIndex {0}, refCount {1}", bitIndex, GlobalLockRefCount[bitIndex]);
            }
